...
CATALOG: dict[str, list[dict]] = {
    ...
    "walled": [
        ...
        {"name": "xiaohongshu_read", "tier": "walled", "script": "walled/xiaohongshu_read.py",
         "description": "小红书 note detail reader (body + comments + metadata.images) — port 9223; URLs MUST carry xsec_token (use xiaohongshu_search output)"},
        {"name": "xiaohongshu_view", "tier": "walled", "script": "walled/xiaohongshu_view.py",
         "description": "小红书 note image downloader — plain HTTP with the Referer the xhscdn/rednotecdn CDN requires (no CDP needed if you already have URLs); feed it metadata.images from xiaohongshu_read, or --note-url to read+download in one step (that mode needs the 9223 Chrome)"},
